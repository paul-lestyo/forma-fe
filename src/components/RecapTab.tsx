...
export const RecapTab: React.FC = () => {
  ...
  const fetchRecap = async (month: string, offset: number) => {
    ...
    try {
      ...
      // Auto-select today if in this month, or day with highest EXP
  ...
  // Calculate blank offset cells before the 1st of the month (1=Mon..7=Sun)
  ...
  return (
    <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-xl shadow-slate-200/40 max-w-sm mx-auto min-h-[440px] flex flex-col justify-between space-y-4">
      <div className="space-y-4">
        ...
        {/* GitHub-style Monthly Heatmap Activity */}
        <div className="space-y-2.5 pt-2 border-t border-slate-100">
          ...
          <div className="grid grid-cols-7 gap-1 text-center">
            {['Sen', 'Sel', 'Rab', 'Kam', 'Jum', 'Sab', 'Min'].map((dayName, idx) => (
              <span key={idx} className="text-[9px] font-semibold text-slate-400">
                {dayName}
              </span>
            ))}
          </div>

          {/* Monthly Heatmap 7-Col Grid */}
          <div className="grid grid-cols-7 gap-1">
            {/* Blank offset cells for start of month */}
            {blankCells.map((_, i) => (
              <div key={`blank-${i}`} className="aspect-square rounded-[5px] bg-transparent" />
            ))}

            {/* Actual day cells with intensity levels */}
            {monthlyActivity.map((d) => {
              ...
              if (d.level === 1) {
                colorClass = 'bg-slate-300 text-slate-800 hover:bg-slate-400';
              } else if (d.level === 2) {
                colorClass = 'bg-slate-500 text-white hover:bg-slate-600';
              } else if (d.level === 3) {
                colorClass = 'bg-slate-700 text-white hover:bg-slate-800';
              } else if (d.level === 4) {
                colorClass = 'bg-slate-900 text-white hover:bg-black shadow-xs';
              }
              ...
              return (
                <button
                  ...
                  className={`aspect-square rounded-[5px] flex items-center justify-center text-[9px] font-medium transition-all duration-150 select-none ${colorClass} ${
                    isSelected ? 'ring-2 ring-slate-900 ring-offset-1 font-bold z-10' : ''
                  }`}
          ...
          <div className="flex items-center justify-between text-[10px] text-slate-400 pt-1">
            <span className="font-medium">Total: +{data?.total_exp_month || 0} EXP</span>
